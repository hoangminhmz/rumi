...
<div class="swipe-page">
    ...
    <div style="display: flex; gap: 0.5rem; margin-bottom: 1.5rem; align-items: center;">
        ...
        <button id="filterBtn" class="btn btn-outline" style="padding: 0.75rem 1rem; border-radius: 12px;">
...
<!-- Filter Modal -->
<div id="filterModal" class="match-modal" style="display: none;">
    <div class="match-content" style="max-width: 500px;">
        <h2 class="match-title" style="font-size: 1.5rem; margin-bottom: 1rem;">🔍 Bộ lọc</h2>

        <div style="text-align: left; max-height: 60vh; overflow-y: auto; margin-bottom: 1.5rem;">
            <!-- Budget Filter (for find_room mode) -->
            <div id="budgetFilter" style="margin-bottom: 1.5rem; display: <?= $searchMode === 'find_room' ? 'block' : 'none' ?>;">
                <label style="display: block; font-weight: 600; margin-bottom: 0.5rem; color: var(--color-gray-700);">
                    💰 Ngân sách (VND/tháng)
                </label>
                <div style="display: flex; gap: 0.5rem; align-items: center;">
                    <input type="number" id="budgetMin" placeholder="Tối thiểu"
                           style="flex: 1; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 8px;"
                           value="<?= $userPreferences['budget_min'] ?? 0 ?>">
                    <span>-</span>
                    <input type="number" id="budgetMax" placeholder="Tối đa"
                           style="flex: 1; padding: 0.75rem; border: 1px solid #e5e7eb; border-radius: 8px;"
                           value="<?= $userPreferences['budget_max'] ?? 10000000 ?>">
                </div>
            </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                    <button type="button" class="cleanliness-btn" data-value="<?= $i ?>"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        <?= $i ?>
                    </button>
                    <?php endfor; ?>
                </div>
            ...
            <!-- Noise Tolerance Filter -->
            <div style="margin-bottom: 1.5rem;">
                <label style="display: block; font-weight: 600; margin-bottom: 0.5rem; color: var(--color-gray-700);">
                    🔊 Độ chịu đựng tiếng ồn
                </label>
                <div style="display: flex; gap: 0.5rem;">
                    <?php for ($i = 1; $i <= 5; $i++): ?>
                    <button type="button" class="noise-btn" data-value="<?= $i ?>"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        <?= $i ?>
                    </button>
                    <?php endfor; ?>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <button type="button" class="smoking-btn" data-value="any"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        Tất cả
                    </button>
                    <button type="button" class="smoking-btn" data-value="no"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        Không hút
                    </button>
                    <button type="button" class="smoking-btn" data-value="yes"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        Có hút
                    </button>
                </div>
            ...
            <div style="margin-bottom: 1.5rem;">
                ...
                <div style="display: flex; gap: 0.5rem;">
                    <button type="button" class="pets-btn" data-value="any"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        Tất cả
                    </button>
                    <button type="button" class="pets-btn" data-value="no"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        Không nuôi
                    </button>
                    <button type="button" class="pets-btn" data-value="yes"
                            style="flex: 1; padding: 0.75rem; border: 2px solid #e5e7eb; border-radius: 8px; background: white; cursor: pointer; transition: all 0.2s;">
                        Có nuôi
                    </button>
                </div>
        ...
        <div style="display: flex; gap: 1rem;">
            <button class="btn btn-outline" style="flex: 1;" onclick="resetFilters()">Reset</button>
            <button class="btn btn-primary" style="flex: 1;" onclick="applyFilters()">Áp dụng</button>
        </div>
    </div>
</div>
...
let filterState = {
    ...
    cleanliness: <?= $userPreferences['cleanliness'] ?? 'null' ?>,
    noiseTolerance: <?= $userPreferences['noise_tolerance'] ?? 'null' ?>,
    smoking: '<?= isset($userPreferences['smoking']) ? ($userPreferences['smoking'] ? 'yes' : 'no') : 'any' ?>',
    pets: '<?= isset($userPreferences['pets']) ? ($userPreferences['pets'] ? 'yes' : 'no') : 'any' ?>'
};
...
function initializeFilterUI() {
    ...
    if (budgetMinInput) budgetMinInput.value = filterState.budgetMin;
    if (budgetMaxInput) budgetMaxInput.value = filterState.budgetMax;

    // Set cleanliness buttons
    document.querySelectorAll('.cleanliness-btn').forEach(btn => {
        const value = parseInt(btn.dataset.value);
        if (value === filterState.cleanliness) {
    ...
    // Set noise tolerance buttons
    document.querySelectorAll('.noise-btn').forEach(btn => {
        const value = parseInt(btn.dataset.value);
        if (value === filterState.noiseTolerance) {
            btn.style.borderColor = 'var(--color-primary)';
            btn.style.background = 'var(--color-primary)';
            btn.style.color = 'white';
        } else {
    ...
    // Set smoking buttons
    document.querySelectorAll('.smoking-btn').forEach(btn => {
        const value = btn.dataset.value;
        if (value === filterState.smoking) {
            btn.style.borderColor = 'var(--color-primary)';
            btn.style.background = 'var(--color-primary)';
            btn.style.color = 'white';
        } else {
    ...
    // Set pets buttons
    document.querySelectorAll('.pets-btn').forEach(btn => {
        const value = btn.dataset.value;
        if (value === filterState.pets) {
            btn.style.borderColor = 'var(--color-primary)';
            btn.style.background = 'var(--color-primary)';
            btn.style.color = 'white';
        } else {
...
// Handle cleanliness button clicks
document.querySelectorAll('.cleanliness-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        const value = parseInt(btn.dataset.value);
        filterState.cleanliness = value;
        initializeFilterUI();
    });
});

// Handle noise tolerance button clicks
document.querySelectorAll('.noise-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        const value = parseInt(btn.dataset.value);
        filterState.noiseTolerance = value;
        initializeFilterUI();
    });
});

// Handle smoking button clicks
document.querySelectorAll('.smoking-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        const value = btn.dataset.value;
        filterState.smoking = value;
        initializeFilterUI();
    });
});

// Handle pets button clicks
document.querySelectorAll('.pets-btn').forEach(btn => {
    btn.addEventListener('click', () => {
        const value = btn.dataset.value;
        filterState.pets = value;
        initializeFilterUI();
    });
});
...
function applyFilters() {
    ...
    if (budgetMinInput) filterState.budgetMin = parseInt(budgetMinInput.value) || 0;
    if (budgetMaxInput) filterState.budgetMax = parseInt(budgetMaxInput.value) || 10000000;
    ...
    if (filterState.cleanliness !== null) {
        params.set('cleanliness', filterState.cleanliness);
    }

    if (filterState.noiseTolerance !== null) {
        params.set('noise_tolerance', filterState.noiseTolerance);
    }

    if (filterState.smoking !== 'any') {
        params.set('smoking', filterState.smoking === 'yes' ? '1' : '0');
    } else {
        params.delete('smoking');
    }

    if (filterState.pets !== 'any') {
        params.set('pets', filterState.pets === 'yes' ? '1' : '0');
    } else {
        params.delete('pets');
    }
...
function resetFilters() {
    ...
    // Redirect to page without filter params
